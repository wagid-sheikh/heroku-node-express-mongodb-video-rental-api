...
router.get('/', auth, asyncMiddleWare(async (req, res) =>{
    ...
    (genres.length > 0) ? res.status(200).send(genres): res.status(404).json('No Records Found');
}));
router.get('/:id', [auth, validateObjectId], asyncMiddleWare(async (req, res) =>{
    const genre = await Genre.findById(req.params.id);
    (genre) ? res.status(200).send(genre): res.status(404).json('No such Record Found');
}));
